...
![Cartographic visualisation of the identified clusters of hotels, AirBnBs, and Cultural venues](figures/5cluster_map(with inset)_dupsremoved.png)
...
K-means clustering is a technique used to minimise distance between centroids of data points, allowing clusters to be identified iterativley. As each centroid is updated the distance is recauculated, running until the centroid no longer changes. A key advantage of k-means clustering is the ease of use and efficency of computation, though can be biased by irregular patterns within the data. 
